"""CLI orchestration for the measured SGLang FullKV backend."""
...
from mosaickv.manifest import ArtifactProvenance, ManifestError, RunManifestWriter, sha256_bytes
...
def resolve_sglang_config(args: argparse.Namespace) -> RunConfig | None:
    """Resolve YAML/TOML or direct SGLang flags without importing SGLang."""

    if args.config is not None:
        config = load_config(str(args.config))
        return config if config.execution.backend is Backend.SGLANG else None
    if args.model is None or str(args.backend) != "sglang":
        return None
...
def _resume_payload(
    *,
    ...
    if (
        ...
        raise ManifestError("existing SGLang manifest does not match run/config/raw output")
...
def run_sglang_evaluation(args: argparse.Namespace, config: RunConfig) -> tuple[int, JsonObject]:
    """Execute SGLang FullKV with streaming, cache, and memory measurements."""

    options = SGLangRuntimeOptions(
        tensor_parallel_size=int(args.tensor_parallel_size),
        mem_fraction_static=float(args.sglang_mem_fraction_static),
        context_length=cast("int | None", args.sglang_context_length),
    ...
    runtime = SGLangFullKVModel.from_config(config, options, trace_directory=trace_root)
    try:
        if task.local_scorer is not None:
            summary: JsonObject = (
        ...
        runtime.verify_request_isolation()
    finally:
        runtime.close()

    engine = runtime.runner.engine_metadata
    ...
    RunManifestWriter().write(
        ...
        run_id=run_id,
        execution_metadata={
            "engine_execution_mode": engine.get("execution_mode", "unavailable"),
            "cuda_graph": engine.get("cuda_graph", "unavailable"),
            "overlap_schedule": engine.get("overlap_schedule", "unavailable"),
        ...
        attention_implementation_override=(
            "sglang_" + str(engine.get("attention_backend", "unavailable")).lower()
        ),
    )
    rows = JsonlResultStore(raw_path).results(run_id=run_id)
    failed = sum(row.status.value == "failed" for row in rows)
